...
// Account-level ACH filter configuration + whitelist + dual control (Firestore async)
...
async function ensureAccounts() {
  const accts = await getTable('accounts');
  if (accts.length === 0) {
    const defaults = [
    ...
    for (const a of defaults) await insert('accounts', a);
  }
}
...
router.get('/', async (req, res) => {
  try {
    ...
    const accounts = await queryAll('accounts', null, { orderBy: 'account_name', desc: false });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await update('accounts', a => a.account_id === req.params.id, () => ({
    ...
    await insert('audit_logs', { transaction_id:null, event_type:'rule_updated', event_summary:`Account ${req.params.id} filter config updated: mode=${filter_mode||'unchanged'}`, event_data:req.body, actor:'HUMAN', severity:'info' });
...
router.post('/:id/whitelist', async (req, res) => {
  try {
    ...
    if (!existing.includes(company_id)) {
      await update('accounts', a => a.account_id === req.params.id, a => ({ authorized_company_ids: [...(a.authorized_company_ids||[]), company_id] }));
    }
    await insert('acl_filter_rules', { account_id:req.params.id, company_id, company_name:company_name||company_id, max_amount:parseFloat(max_amount)||null, frequency_limit:frequency_limit||null, notes:notes||null, is_active:true });
    await insert('audit_logs', { transaction_id:null, event_type:'rule_updated', event_summary:`Company ${company_id} added to ACH allow list for account ${req.params.id}`, event_data:{ company_id, max_amount, frequency_limit }, actor:'HUMAN', severity:'info' });
    res.json({ success:true, message:`${company_id} added to allow list` });
  } catch(e) { res.status(500).json({ success:false, error:e.message }); }
});

// DELETE /api/accounts/:id/whitelist/:company_id
router.delete('/:id/whitelist/:company_id', async (req, res) => {
  try {
    await update('accounts', a => a.account_id === req.params.id, a => ({
      authorized_company_ids: (a.authorized_company_ids||[]).filter(c => c !== req.params.company_id)
    }));
    await update('acl_filter_rules', r => r.account_id === req.params.id && r.company_id === req.params.company_id, () => ({ is_active:false }));
