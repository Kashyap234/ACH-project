...
// Issued Check Register — upload, manage, match incoming checks (Firestore async)
...
router.post('/:account_id/bulk', async (req, res) => {
  try {
...
router.post('/:account_id/match', async (req, res) => {
  try {
    ...
    await insert('audit_logs', { transaction_id:null, event_type:'risk_flagged', event_summary:`Check ${check_serial_number} match: ${matchResult.result} — ${matchResult.details}`, event_data:{ check_serial_number, amount, payee_name, matchResult }, actor:'AI', severity: matchResult.result==='FULL_MATCH'?'info':'warning' });
...
router.put('/:account_id/:check_id/void', async (req, res) => {
  try {
    const { reason } = req.body;
    await update('check_register', r => r.id === parseInt(req.params.check_id) && r.account_id === req.params.account_id, () => ({ status:'voided', void_reason:reason||'Voided by user' }));
